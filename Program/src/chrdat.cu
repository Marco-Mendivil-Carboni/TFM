...
cngeom::cngeom(parmap &par) // parameters
    : R_n{par.get_val<float>("nucleus_radius", 0.0)},
      ...
      R_b{par.get_val<float>("bleb_radius", 0.0)}, noc{cos(asin(R_o / R_n))},
      boc{cos(asin(R_o / R_b))}, nod{sqrt(R_n * R_n - R_o * R_o)},
      bod{sqrt(R_b * R_b - R_o * R_o)}, d_b{nod + bod}, d_m{d_b + R_b}
{
  // check parameters
  if (!(0.0 < R_n && R_n < 100.0))
  {
    throw error("nucleus_radius out of range");
  }
  if (!(0.0 <= R_o && R_o < R_n))
  {
...
{
  // read chromatin data
  uint header[18]; // frame header
  bin_inp_f.read(reinterpret_cast<char *>(header), sizeof(header));
  i_f = header[14];
  t = reinterpret_cast<float &>(header[16]);
  for (uint i_p = 0; i_p < N; ++i_p) // particle index
  {
